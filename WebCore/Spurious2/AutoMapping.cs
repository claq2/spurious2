...
public class AutoMapping : Profile
{
    public AutoMapping()
    {
        this.CreateMap<Core2.Stores.Store, Store>()
            ...
            {
                new () { AlcoholType = Core2.Stores.AlcoholType.Beer.ToString(), Volume = (s.BeerVolume ?? 0) / 1000 },
                new () { AlcoholType = Core2.Stores.AlcoholType.Spirits.ToString(), Volume = (s.SpiritsVolume ?? 0) / 1000 },
                new () { AlcoholType = Core2.Stores.AlcoholType.Wine.ToString(), Volume = (s.WineVolume ?? 0) / 1000 },
            }));
