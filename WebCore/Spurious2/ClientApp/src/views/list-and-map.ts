...
import { Inventory, Store, Subdivision, SubdivisionsApi, StoresApi } from "api";
...
export class ListAndMap {
  ...
  private densityShortName!: string;
  ...
  constructor(
    ...
    private subdivisionsApi: SubdivisionsApi,
  ...
  async activate(params: any, routeConfig: RouteConfig) {
    ...
    this.densityShortName = routeConfig.settings.shortname;
  ...
  attached() {
    ...
    this.map.events.addOnce("ready", async () => {
      ...
      const lineLayer = new layer.LineLayer(this.datasource, undefined, {
        strokeColor: "black",
        strokeWidth: 0.7,
      });
      ...
      const subdivisions: Subdivision[] =
        await this.subdivisionsApi.getSubdivisionsByDensity({
          name: this.densityShortName,
        });
